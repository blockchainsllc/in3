...
static const char UnityStrIgnore[] = "\033[31mIGNORE\033[00m";
#else
...
void UnityAssertEqualIntArray(UNITY_INTERNAL_PTR          expected,
  ...
  while (elements--) {
    ...
    if (expect_val != actual_val) {
      if (style & UNITY_DISPLAY_RANGE_UINT && length < sizeof(expect_val)) { /* For UINT, remove sign extension (padding 1's) from signed type casts above */
        UNITY_INT mask = 1;
