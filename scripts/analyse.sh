...
docker run \
  --rm \
  -v $(pwd):$(pwd) \
  docker.slock.it/build-images/cmake:clang13 \
  /bin/bash -c "cd $(pwd); rm -rf build; mkdir build; cd build; scan-build-13 $OPTS cmake -DEVM_GAS=true -DCMAKE_BUILD_TYPE=DEBUG -DIN3_SERVER=true .. &&  scan-build-13 $OPTS --exclude ../c/src/third-party --force-analyze-debug-code -o report  make -j8"
cd scripts
