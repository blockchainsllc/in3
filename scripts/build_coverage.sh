...
LCOV="../scripts/lcov_report.sh | xargs llvm-cov-16 report && ../scripts/lcov_report.sh | xargs llvm-cov-16 show -show-line-counts-or-regions -output-dir=all-merged -format=html  && ../scripts/lcov_report.sh | xargs llvm-cov-16 export  -format=lcov  > lcov.info"
cd ..
docker run \
  --rm \
  -v $(pwd):/src \
