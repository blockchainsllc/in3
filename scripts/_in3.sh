...
subcmds=(
    ...
    'sendtransaction: signs and sends a transaction to a btc node <signer> <txoutputs> <utxos>'
    ...
    'eth_accounts: alias to signer_ids'
    ...
    'in3_derive_signer: derrives a new signer <path> <seed_id>'
    ...
    'in3_verifySignature: verifies a signature of a wallet <msg> <account> <sig> <sigtype> <blockNumber>'
    'signer_ids: returns a array of signer_ids the client is able to sign with'
    'in3_abiDecode: based on the  <signature> <data> <topics>'
    'in3_abiEncode: based on the  <signature> <parameters>'
    ...
    'zksync_account_history: returns the history of transaction for a given account <account> <ref_start> <limit>'
...
'--finality=[In case of a Proof of Authority chain finality indicates the amount of blocks needed to reach finali...]:finality:()'
'-f[In case of a Proof of Authority chain finality indicates the amount of blocks needed to reach finali...]:finality:()'
...
'--gasPrio=[the factor in percent to be used when calculating the gasPrice (50 = half of average gasPrice, 200 =...]:gasPrio:()'
'-g[the factor in percent to be used when calculating the gasPrice (50 = half of average gasPrice, 200 =...]:gasPrio:()'
'--keepIn3[if true, requests sent to the input sream of the comandline util will be send the responses in the s...]'
'-kin3[if true, requests sent to the input sream of the comandline util will be send the responses in the s...]'
...
'--replaceLatestBlock=[Defines the number of blocks that will be substracted from latest]:replaceLatestBlock:()'
'-l[Defines the number of blocks that will be substracted from latest]:replaceLatestBlock:()'
...
'--nodeProps=[Defines the requirements of an in3 node in order to be used by this instance]:nodeProps:()'
...
'--hd.seed=[the seed (64 bytes hex)]:seed:()'
'--hd.seed_phrase=[the seedphrase]:seed_phrase:()'
'--hd.seed_password=[a optional seed]:seed_password:()'
'--hd.paths=[the derrivation paths, if not specified the default will be used]:paths:()'
'--key=[The client key to sign in3 requests]:key:()'
'-k[The client key to sign in3 requests]:key:()'
'--pk=[registers raw private keys as signers for blockchain transactions]:pk:()'
'-pk[registers raw private keys as signers for blockchain transactions]:pk:()'
