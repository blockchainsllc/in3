...
subcmds=(
    ...
    'zksync_deposit: sends a L1-Transaction to add to the deposit <amount> <token> <erc20Approve:bool> <account>'
    'zksync_transfer: transfers the given amount from the configured account to another account in L2 <to> <amount> <token> <account>'
    'zksync_withdraw: transfers the given amount from the configured account in L2 to another account in L1 <to> <amount> <token> <account>'
    'zksync_set_key: sets the configured key as signer for the account  args: <token_for_fee>'
    'zksync_emergency_withdraw: sends a L1 Tx to withdraw all tokens from L2 <token>'
    'zksync_sync_key: calculates and returns the pk used to sign zksync-tx (using babyjupjup curve)'
    'zksync_aggregate_pubkey: calculates the combined musig public key for the concatinated public keys (each with 32 bytes)'
    'zksync_pubkeyhash: returns the pubkeyhash of the signer based on the config'
    'zksync_pubkey: returns the compact pubkey(32 bytes) of the signer based on the config or from the <seed-arg>, if given'
    'zksync_account_address: returns address of the zksync-account based on the config'
    'zksync_account_info: returns the current balance and state of the account'
    'zksync_tokens: returns a list of all available tokens'
    'zksync_sign: sign the given <bytes> with the syncKey. If the -zms and -zmu are passed, it will use musig schnirr sigs to create MPC-Signature'
    'zksync_verify: verifies the signature of the <msg> <signature>'
