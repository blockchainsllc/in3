...
#define UPDATE_SUBCALL_GAS(evm, parent, address, code_address, caller, gas, mode, value, l_value)                       \
  do {                                                                                                                  \
    ...
    if (!address) {                                                                                                     \
      ...
      res        = evm_create_account(&evm, evm.call_data.data, evm.call_data.len, code_address, caller, &new_account); \
      no_address = true;                                                                                                \
    }                                                                                                                   \
    else { /* if no code was already set for execution */                                                               \
      account_t* ac = NULL;                                                                                             \
      res           = evm_get_account(parent, code_address, 0, &ac);                                                    \
      if (res == 0 && ac) evm.code = ac->code;                                                                          \
    }                                                                                                                   \
    if (res == 0 && !big_is_zero(value, l_value)) {                                                                     \
      if (mode == EVM_CALL_MODE_STATIC)                                                                                 \
        ...
        res = transfer_value(&evm, parent->address, evm.address, value, l_value, c_xfer, false);                        \
      }                                                                                                                 \
    }                                                                                                                   \
