...
in3_ret_t btc_serialize_block_header(d_token_t* data, uint8_t* block_header) {
  rev_hex(d_get_stringk(data, key("versionHex")), block_header, 4);
  rev_hex(d_get_stringk(data, key("previousblockhash")), block_header + 4, 32);
  rev_hex(d_get_stringk(data, key("merkleroot")), block_header + 36, 32);
  rev_copyl(block_header + 68, d_to_bytes(d_get(data, key("time"))), 4);
  rev_hex(d_get_stringk(data, key("bits")), block_header + 72, 4);
