...
static in3_ret_t handle_in3_addRawKey(in3_rpc_handle_ctx_t* ctx) {
  ...
  RPC_ASSERT(d_len(ctx->params) <= 2, "%s only accepts %u arguments.", FN_IN3_ADDRAWKEY, 2);
...
static in3_ret_t handle_in3_addJsonKey(in3_rpc_handle_ctx_t* ctx) {
  ...
  RPC_ASSERT(d_len(ctx->params) <= 2, "%s only accepts %u arguments.", FN_IN3_ADDJSONKEY, 2);
...
static in3_ret_t handle_in3_addMnemonic(in3_rpc_handle_ctx_t* ctx) {
  ...
  RPC_ASSERT(d_len(ctx->params) <= 4, "%s only accepts %u arguments.", FN_IN3_ADDMNEMONIC, 4);
...
static in3_ret_t handle_in3_derive_signer(in3_rpc_handle_ctx_t* ctx) {
  ...
  RPC_ASSERT(d_len(ctx->params) <= 2, "%s only accepts %u arguments.", FN_IN3_DERIVE_SIGNER, 2);
...
in3_ret_t pk_signer_rpc(in3_rpc_handle_ctx_t* ctx) {
  ...
  TRY_RPC(FN_IN3_ADDRAWKEY, handle_in3_addRawKey(ctx))
#endif

#if !defined(RPC_ONLY) || defined(RPC_IN3_ADDJSONKEY)
  TRY_RPC(FN_IN3_ADDJSONKEY, handle_in3_addJsonKey(ctx))
#endif

#if !defined(RPC_ONLY) || defined(RPC_IN3_ADDMNEMONIC)
  TRY_RPC(FN_IN3_ADDMNEMONIC, handle_in3_addMnemonic(ctx))
#endif

#if !defined(RPC_ONLY) || defined(RPC_IN3_DERIVE_SIGNER)
  TRY_RPC(FN_IN3_DERIVE_SIGNER, handle_in3_derive_signer(ctx))
#endif

#if !defined(RPC_ONLY) || defined(RPC_SIGNER_IDS)
  TRY_RPC(FN_SIGNER_IDS, signer_ids(ctx))
#endif

#if !defined(RPC_ONLY) || defined(RPC_ETH_ACCOUNTS)
  TRY_RPC(FN_ETH_ACCOUNTS, eth_accounts(ctx))
#endif
