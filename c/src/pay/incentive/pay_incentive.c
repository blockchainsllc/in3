...
static bytes_t* payed_addresses = NULL;

#ifdef THREADSAFE
#if defined(_MSC_VER) || defined(__MINGW32__)
static HANDLE lock_payed = NULL;
static void   _lock_payed() {
  if (!lock_payed) {
    HANDLE p = CreateMutex(NULL, FALSE, NULL);
    if (InterlockedCompareExchangePointer((PVOID*) &lock_payed, (PVOID) p, NULL)) CloseHandle(p);
  }
  MUTEX_LOCK(lock_payed);
}
#define LOCK_PAYED(code)         \
  {                              \
    _lock_payed();               \
    code                         \
        MUTEX_UNLOCK(lock_payed) \
  }
#else
static pthread_mutex_t lock_payed = PTHREAD_MUTEX_INITIALIZER;
#define LOCK_PAYED(code)         \
  {                              \
    MUTEX_LOCK(lock_payed);      \
    code                         \
...
static in3_ret_t pay_get_payed_addresses(in3_ctx_t* ctx, bytes_t* dst) {
  if (payed_addresses) {
  }
...
static in3_ret_t pay_get_payed(in3_ctx_t* ctx, address_t adr) {
  in3_cache_ctx_t c = {.content = NULL, .ctx = ctx, .key = alloca(20)};
  sprintf(c.key, "payed_%d", (uint32_t) ctx->client->chain.chain_id);
  TRY(in3_plugin_execute_first_or_none(ctx, PLGN_ACT_CACHE_GET, c))
  if (c.content) {
