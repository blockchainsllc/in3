...
in3_ret_t zksync_musig_receive_pre_commit(zksync_config_t* conf, in3_rpc_handle_ctx_t* ctx, d_token_t* params) {
  expect_params_eq(2);
  if (d_len(params + 2) != 32) return ctx_set_error(ctx->ctx, "invalid incomming precommit", IN3_EINVAL);
  int pos = get_pubkey_pos(conf);
  TRY(pos)
  int     in_pos = pos == 0 ? 1 : 0;
  uint8_t all_precommit[64], result[64];
  bytes_t seed = d_to_bytes(params + 1);
  memcpy(all_precommit + in_pos * 32, d_to_bytes(params + 1).data, 32);
  zkcrypto_signer_t signer = zkcrypto_signer_new(conf->musig_pub_keys, pos);
  in3_ret_t         r      = zkcrypto_signer_compute_precommitment(signer, seed, all_precommit + pos * 32);
  memcpy(result, all_precommit + pos * 32, 32);
  r = r < 0 ? r : zkcrypto_signer_receive_precommitment(signer, bytes(all_precommit, 64), result + 32);
  zkcrypto_signer_free(signer);
  TRY(r)
  return in3_rpc_handle_with_bytes(ctx, bytes(result, 64));
}
