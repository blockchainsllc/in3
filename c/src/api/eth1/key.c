...
in3_ret_t decrypt_key(d_token_t* key_data, char* password, bytes32_t dst) {
  if (d_get_intk(key_data, K_VERSION) != 3) return IN3_EVERS;
  d_token_t* crypto     = d_get(key_data, key("crypto"));
  char*      kdf        = d_get_stringk(crypto, key("kdf"));
  d_token_t* kdf_params = d_get(crypto, key("kdfparams"));
  if (!crypto || !kdf || !kdf_params) return IN3_EINVALDT;
  int      klen      = d_get_intk(kdf_params, key("dklen"));
  char*    salt_hex  = d_get_stringk(kdf_params, key("salt"));
  ...
  if (strcmp(kdf, "scrypt") == 0) {
    ...
    if (libscrypt_scrypt((const uint8_t*) password, strlen(password), salt.data, salt.len,
                         d_get_longk(kdf_params, key("n")), d_get_intk(kdf_params, key("r")), d_get_longk(kdf_params, key("p")), aeskey, klen))
      return IN3_EPASS;
#else
    return IN3_ENOTSUP;
#endif
    ...
    if (!kdf_params || strcmp(d_get_stringk(kdf_params, key("prf")), "hmac-sha256")) return IN3_ENOTSUP;
    if (strcmp(d_get_stringk(crypto, key("cipher")), "aes-128-ctr")) return IN3_ENOTSUP;
    pbkdf2_hmac_sha256((const uint8_t*) password, strlen(password), salt.data, salt.len, d_get_intk(kdf_params, key("c")), aeskey, klen);
  }
  else
    return IN3_ENOTSUP;

  bytes_t  cipher = bytes(cipher_data, hex_to_bytes(d_get_stringk(crypto, key("ciphertext")), -1, cipher_data, 64));
  ...
  hex_to_bytes(d_get_stringk(crypto, key("mac")), -1, mac_verify, 32);
  if (memcmp(mac, mac_verify, 32)) return IN3_EPASS;

  // aes-128-ctr
  aes_init();
  aes_encrypt_ctx cx[1];
  char*           iv_hex  = d_get_stringk(d_get(crypto, key("cipherparams")), key("iv"));
