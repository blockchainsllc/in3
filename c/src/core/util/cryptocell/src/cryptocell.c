...
int derive_ek_sym_aes_enc_dec(uint8_t *key_label, uint32_t label_size, uint8_t *key_out)
{
    ...
    status = crypto_deinit(key_id_out);
    if (status != PSA_SUCCESS)
    {
        printk("psa crypto de-initialization completed\n");
        res = ERROR;
        return res;
    }
    return res;
...
int store_pk_identity_keyslot_kmu(uint32_t slot, uint8_t *key)
{
    ...
    if (!nrf_cc3xx_platform_identity_key_is_stored(slot))
    {
        ...
        status = nrf_cc3xx_platform_identity_key_store(NRF_KMU_SLOT_KIDENT, key);
        if (status == NRF_CC3XX_PLATFORM_SUCCESS)
        {
            printk("success storing secret key to kmu slot %d\n", slot);
        }
        return status;
    }
    return status;
...
int load_pk_identity_keyslot_kmu(uint32_t slot, uint8_t *key)
{
    ...
    if (nrf_cc3xx_platform_identity_key_is_stored(slot))
    {
        ...
        if (status != NRF_CC3XX_PLATFORM_SUCCESS)
        {
            printk("Key retreival from kmu slot %d failed\n", slot);
            return status;
        }
    }
    return status;
