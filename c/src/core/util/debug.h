...
#define EXPECT_TOK_ADDR(token)        EXPECT_TOK(token, d_is_bytes(token) && d_to_bytes(token).len == 20, "expected address")
#define EXPECT_TOK_B256(token)        EXPECT_TOK(token, d_is_bytes(token) && d_to_bytes(token).len == 32, "expected 256 bit data")
