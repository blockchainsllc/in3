...
#define CHECK_PARAMS_LEN(ctx, params, len) \
  if (d_type(params) != T_ARRAY || d_len(params) < len) return ctx_set_error(ctx, "arguments need to be a array with at least " #len " arguments", IN3_EINVAL);
#define CHECK_PARAM_TYPE(ctx, params, index, type) \
  if (d_type(d_get_at(params, index)) != type) return ctx_set_error(ctx, "argument at index " #index " must be a " #type, IN3_EINVAL);
#define CHECK_PARAM_NUMBER(ctx, params, index)                                                       \
  switch (d_type(d_get_at(params, index))) {                                                         \
    case T_INTEGER:                                                                                  \
    case T_BYTES: break;                                                                             \
    default: return ctx_set_error(ctx, "argument at index " #index " must be a number", IN3_EINVAL); \
  }
...
#define CHECK_PARAM_MAX_LEN(ctx, params, index, len) \
  if (d_len(d_get_at(params, index)) > len) return ctx_set_error(ctx, "argument at index " #index " must be smaller than " #len, IN3_EINVAL);
#define CHECK_PARAM_MIN_LEN(ctx, params, index, len) \
  if (d_len(d_get_at(params, index)) < len) return ctx_set_error(ctx, "argument at index " #index " must be at least " #len, IN3_EINVAL);
#define CHECK_PARAM(ctx, params, index, cond)                                                             \
  {                                                                                                       \
    const d_token_t* val = d_get_at(params, index);                                                       \
    if (!(cond)) return ctx_set_error(ctx, "argument at index " #index " must match " #cond, IN3_EINVAL); \
  }
