...
static d_token_t* get_account(in3_vctx_t* vc, d_token_t* accounts, uint8_t* address) {
  ...
  for (i = 0, t = accounts + 1; i < d_len(accounts); i++, t = d_next(t)) {
    if (memcmp(d_get_bytesk(t, K_ADDRESS), address, 20) == 0)
      return t;
  }
  ...
  switch (evm_key) {
    ...
    case EVM_ENV_STORAGE:
      ...
      for (i = 0, t2 = t + 1; i < d_len(t); i++, t2 = d_next(t2)) {
        res = d_get_bytesk(t2, K_KEY);
        if (!res) return EVM_ERROR_INVALID_ENV;
        if (big_cmp(in_data, in_len, res->data, res->len) == 0) {
          res = d_get_bytesk(t2, K_VALUE);
          if (!res) return EVM_ERROR_INVALID_ENV;
          *out_data = res->data;
          return res->len;
        }
