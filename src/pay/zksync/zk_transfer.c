...
in3_ret_t zksync_transfer(zksync_config_t* conf, in3_rpc_handle_ctx_t* ctx, zk_msg_type_t type) {
  ...
  if (!cached) {
    ...
    cached        = in3_cache_add_entry(&ctx->req->cache, NULL_BYTES, bytes((void*) sb.data, strlen(sb.data))); // NOSONAR - this function expects null-terminated string which was checked prior to calling it
  ...
  if (ret == IN3_OK && cached && cached->value.data) {
    ...
    sb_add_range(sb, (void*) cached->value.data, 0, start_sig ? ((size_t) (start_sig - cached->value.data)) : (strlen((void*) cached->value.data) - 177)); // NOSONAR ached->value.data is a safe nullterminated string
