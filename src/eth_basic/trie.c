...
static trie_codec_t rlp_codec = {
    .decode_item  = rlp_decode_in_list,
...
static trie_node_t* trie_node_new(uint8_t* data, size_t len, uint8_t is_embedded) {
...
static trie_node_t* trie_node_create_ext(trie_t* trie, uint8_t* nibbles, bytes_t* value) {
    bytes_builder_t* bb = bb_new();
    bytes_t empty= {.data=NULL,.len=0};
        ...
        switch (n->type) {
          ...
          case NODE_LEAF:
             ...
             if (*node_path==0XFF) 
              ...
              // so this is a leaf with a longer path and we try to set a value with here
              // so we create a branch and set the leaf
              trie_node_set_path(n,path+1);
              b = trie_node_create_branch(trie, value);
              trie_node_set_item(n,*node_path, key_as_data( update_db(trie, n, false)));
              n = b;
             }
             break;
           case NODE_EXT:  
             ...
             if (nibble_len(rel_path)==1)
               ...
               trie_node_set_item(b,*rel_path, &tmp );
                ...
                trie_node_set_item(b,*rel_path, key_as_data( update_db(trie, n, false)) );
             }
             n = b;
             break;
        }
        ...
        switch(n->type) {
           ...
           case NODE_BRANCH:
             if (trie_node_get_item(n,first).len==0) 
               ...
               trie_node_set_item(n,first, key_as_data( update_db( trie, trie_node_create_leaf(trie, path+1,value), false)) );
             else {
               // handle the next node
               b = rlp_decode(&n->items,first,&tmp)==1
                   ? get_node( trie, hash_key( &tmp) )
                   : trie_node_new(tmp.data,tmp.len,true);
               trie_node_set_item(n,first, key_as_data( handle_node(trie, b, path+1, value, false )));
             }
             break;
             ...
             if (matching == nibble_len(node_path)) {  // next element fits so we can update next node 
                if (n->type==NODE_LEAF)                // for Leaf: we simply replace the leaf-value
                  ...
                  b = rlp_decode(&n->items,1,&tmp)==1
                      ? get_node( trie, hash_key( &tmp) )
                      : trie_node_new(tmp.data,tmp.len,true);
                  trie_node_set_item(n,1, key_as_data( handle_node( trie, b, path + matching, value, false) ));
                }
             }
