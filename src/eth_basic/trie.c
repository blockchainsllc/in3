...
static int trie_node_value_from_nibbles(trie_node_type_t type, uint8_t *val,
                                        bytes_t *dst) {
  ...
  dst->len = blen;
  dst->data[0] =
      (type == NODE_EXT ? 0 : 2 + odd) << 4 | (odd == 0 ? 0 : val[0]);
...
static trie_node_t *trie_node_create_leaf(trie_t *trie, uint8_t *nibbles,
                                          bytes_t *value) {
  bytes_builder_t *bb    = bb_new();
  bytes_t          empty = {.data = NULL, .len = 0};
