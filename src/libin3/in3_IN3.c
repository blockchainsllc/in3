...
JNIEXPORT jobject JNICALL Java_in3_IN3_getProof(JNIEnv *env, jobject ob) {
    ...
    switch (in3->proof) {
        case PROOF_NONE:
          val = (*env)->GetStaticFieldID(env,enum_clazz , "none", "Lin3/Proof;"); break;
        case PROOF_STANDARD:
          val = (*env)->GetStaticFieldID(env,enum_clazz , "standard", "Lin3/Proof;"); break;
        case PROOF_FULL:
          val = (*env)->GetStaticFieldID(env,enum_clazz , "full", "Lin3/Proof;"); break;
