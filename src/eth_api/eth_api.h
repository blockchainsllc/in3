...
size_t       eth_newFilter(in3_t* in3, in3_filter_opt_t* options);                                    /**< creates a new event filter with specified options and returns its id (>0) on success or 0 on failure; on failure options must be freed explicitly */
