...
static const char*  TX_FIELDS_0[] = {"nonce", "gasPrice", "gas", "to", "value", "data", "v", "r", "s", 0, 0, 0, 0};
static const char*  TX_FIELDS_1[] = {"chainId", "nonce", "gasPrice", "gas", "to", "value", "data", "accessList", "v", "r", "s", 0, 0};
static const char*  TX_FIELDS_2[] = {"chainId", "nonce", "maxPriorityFeePerGas", "maxFeePerGas", "gas", "to", "value", "data", "accessList", "v", "r", "s", 0};
static const size_t MAX_FIELD_LEN = sizeof(TX_FIELDS_2) / sizeof(char*);
...
static in3_ret_t in3_sign_data(in3_rpc_handle_ctx_t* ctx) {
  ...
  else if (sc.account.len == 32) { // in this case we treat the account as private key
  ...
  if (is_eth_sign)
    in3_rpc_handle_with_bytes(ctx, sig_bytes);
  else {
