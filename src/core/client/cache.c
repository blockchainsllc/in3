...
int in3_cache_update_nodelist(in3* c, in3_chain_t* chain) {
    ...
    if (b) {
        ...
        if (jsmnutil_parse_json_range((char*)b->data,b->len, &ctx.tok_req, &tokc)>=0 && in3_client_fill_chain(c,chain, &ctx,ctx.tok_req)>=0)
...
int in3_cache_store_nodelist(in3* c, in3_ctx_t* ctx, in3_chain_t* chain) {
